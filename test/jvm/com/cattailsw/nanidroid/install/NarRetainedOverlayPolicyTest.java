...
public final class NarRetainedOverlayPolicyTest {
    ...
    public void unicodeAndCaseCollisionsUseOneCrossTreeKey()
        ...
        Baseline nested = present("ghost",
                directory(nfd),
                retainedFile(nfd + "/user", 1, 0));
        assertError("INCOMPATIBLE_GHOST_UPDATE",
                NarRetainedOverlayPolicy.build(
                        plan("ghost", "ghost", "0",
                                archiveFile(0, nfc + "/new", 1)),
    ...
    private static NarRetainedOverlayPolicy.Recipe success(
            NarInstallPlan plan, Baseline baseline) {
    ...
    private static void assertError(
            String expected, NarRetainedOverlayPolicy.Result result) {
    ...
    private static void assertEntries(
            NarRetainedOverlayPolicy.Recipe recipe, String... expected) {
    ...
    private static NarInstallPlan plan(String target, String type,
            String refresh, NarArchiveInventory.Entry... entries) {
        Map<String, String> metadata = new LinkedHashMap<String, String>();
        metadata.put("type", type);
        metadata.put("name", "Ghost");
        metadata.put("directory", target);
    ...
    private static NarArchiveInventory.Entry wrapper(
            int ordinal, String path) {
        return archive(ordinal, path, null, true, 0);
    }

    private static NarArchiveInventory.Entry archiveDirectory(
            int ordinal, String path) {
        return archive(ordinal, path + "/", path, true, 0);
    }

    private static NarArchiveInventory.Entry archiveFile(
            int ordinal, String path, long size) {
        return archive(ordinal, path, path, false, size);
    }

    private static NarArchiveInventory.Entry archive(
            int ordinal, String raw, String relative,
    ...
    private static Baseline present(
            String target, BaselineEntry... entries) {
    ...
    private static BaselineEntry retainedFile(
            String path, long size, int ordinal) {
    ...
    private static final class BaselineEntry {
        ...
        private BaselineEntry(
                String path, boolean directory, long size, int ordinal) {
            this.path = path;
