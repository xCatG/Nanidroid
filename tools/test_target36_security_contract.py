...
class Target36SecurityContractTest(unittest.TestCase):
    ...
    def test_service_uses_immutable_pending_intent_and_no_file_uri(self):
        source = (ROOT / "src/com/cattailsw/nanidroid/NanidroidService.java").read_text(encoding="utf-8")
        self.assertIn("private static final int FLAG_IMMUTABLE = 0x04000000;", source)
        self.assertIn("flags |= FLAG_IMMUTABLE;", source)
        self.assertIn("PendingIntent.FLAG_UPDATE_CURRENT", source)
        self.assertIn("startForeground", source)
        self.assertIn("private void finishForegroundWork(int startId)", source)
        self.assertIn("private final Set<Integer> activeForegroundStartIds", source)
        self.assertIn("activeForegroundStartIds.add(startId);", source)
        self.assertIn("activeForegroundStartIds.remove(startId);", source)
        self.assertIn("noForegroundWorkRemains = activeForegroundStartIds.isEmpty();", source)
