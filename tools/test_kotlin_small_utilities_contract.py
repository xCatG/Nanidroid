...
class KotlinSmallUtilitiesContractTest(unittest.TestCase):
    ...
    def test_preference_utility_is_kotlin_and_keeps_java_static_entry_points(self):
        ...
        self.assertFalse((self.root / "src/main/kotlin/com/cattailsw/nanidroid/util/UIUtil.kt").exists())
        self.assertFalse((self.root / "src/main/kotlin/com/cattailsw/nanidroid/util/UIUtil.java").exists())
        self.assertEqual(pref.count("@JvmStatic"), 7)
        for name in (
        ...
        self.assertEqual(pref.count("fun setKey"), 3)
