...
The durable record is the source of truth for archive operations. Archive
...
The current v6 row keeps its historical column count. Column 12 is reserved
and must contain `-`; any other value is unreadable durable state and follows
the quarantine path below. The three supported kinds are `REMOTE_NAR`,
`LOCAL_NAR`, and `NAR_INSTALL`.
...
| Restore running work | Persisted state is `RUNNING` | Preserve any exact-handle attention bit; do not rewrite the record | Start a fresh visual observation window and keep observing the bound job. A surviving exact notification action remains valid, while the in-app prompt is suppressed until the new 30-second deadline. |
| Restore stop request | Persisted state is `CANCEL_REQUESTED` | Preserve any exact-handle attention bit; do not rewrite the record | Immediately repeat cancellation for the exact bound job, or wait for the pending binding and cancel it then; start a fresh visual `Stopping...` observation window without revoking a surviving notification action. |
| Detect unreadable durable state | Stored records fail strict decoding | Atomically quarantine the bounded unreadable payload, reset the primary record set, and persist a recovery-required marker | Pause all new durable writes and show a non-dismissible recovery prompt. Never infer an operation identity or cancel work automatically from corrupt state. |
| Resolve unreadable durable state | Recovery-required marker is present and the user explicitly chooses to discard unreadable history | Clear the quarantine and recovery marker only after the primary reset is known durable | Resume new operations. If clearing fails, keep the prompt visible and remain fail-closed. |
...
| Process restart reconciliation | Persisted active records load with a fresh observation window | Suppress only the in-app prompt from the presented snapshot; retain separately reconciled persisted attentive exact handles | Cancel surviving notifications that do not match persisted attentive exact handles; reuse canonical tag/action identities and actions for matches throughout visual suppression. |
