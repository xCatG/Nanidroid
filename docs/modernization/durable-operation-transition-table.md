...
| Accept retry | Existing attempt is terminal, new attempt ID is greater, and any supplied binding is absent from complete operation history | CAS the terminal record to the new `RUNNING` attempt and binding; carry forward every previously used binding | Prior attempts remain fenced out. A retry never directly or later rebinds any earlier DownloadManager row or Work UUID. |
