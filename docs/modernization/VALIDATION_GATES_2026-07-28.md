...
| `assembleRelease -x verifyLegacyNativeLibraries` | Release Kotlin and Java compilation completed. Assembly failed in `lintVitalRelease`: `res/layout/main.xml:19`, `@id/adview is not a sibling in the same RelativeLayout` (`NotSibling`). | Genuine release-gate failure in a retained legacy XML resource; no lint baseline or source workaround was added. |
