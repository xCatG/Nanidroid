...
D9b1b adds a pure parser for caller-supplied descriptor bytes. The byte array
is cloned at entry. The parser performs no archive, file, install-root, live
tree, extraction, refresh, or event work and remains unwired from `NarUtil`.
