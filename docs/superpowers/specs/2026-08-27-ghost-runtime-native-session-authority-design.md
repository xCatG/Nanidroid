...
primitive. A 204 response, a response without a playable `Value`, an
unscheduled request, or an ownership-certain request failure completes the
authored phase immediately; only a playable response waits for playback. The
application-owned runtime operation then performs the single typed adapter
unload before a current Activity host is told to finish, so Activity recreation
cannot cancel the unload. The dead `Shiori.terminate()` alias is deleted.
If the close request proves ownership uncertain or the runtime is already
poisoned, teardown remains forbidden: the typed fatal terminal is recorded and
the current host is still told to finish without another JNI unload attempt.
