...
public:
	bool nCompare(const ELT *pcsz) const;
	bool nCompareNoCase(const ELT *pcsz) const;
	bool Compare(const ELT *pcsz) const;
	bool CompareNoCase(const ELT *pcsz) const;
	bool Compare(const CBufferRefT<ELT> &) const;
	bool CompareNoCase(const CBufferRefT<ELT> &) const;
...
//
template<class ELT>
CBufferRefT<ELT>::CBufferRefT(const ELT *pcsz, int length) {
	m_pBuffer = (ELT *)pcsz;
	m_nSize	  = length;
}

template<class ELT>
CBufferRefT<ELT>::CBufferRefT(const ELT *pcsz) {
	m_pBuffer = (ELT *)pcsz;
	m_nSize	  = 0;

	if(pcsz != 0)
		while(m_pBuffer[m_nSize] != 0)
			m_nSize++;
}

template<class ELT>
bool CBufferRefT<ELT>::nCompare(const ELT *pcsz) const {
...
template<class ELT>
bool CBufferRefT<ELT>::nCompareNoCase(const ELT *pcsz) const {
	for(int i = 0; i < m_nSize; i++) {
		if(m_pBuffer[i] != pcsz[i]) {
...
template<class ELT>
inline bool CBufferRefT<ELT>::Compare(const ELT *pcsz) const {
	return nCompare(pcsz) ? 1 : (int)pcsz[m_nSize];
}

template<class ELT>
inline bool CBufferRefT<ELT>::CompareNoCase(const ELT *pcsz) const {
	return nCompareNoCase(pcsz) ? 1 : (int)pcsz[m_nSize];
}

template<class ELT>
inline bool CBufferRefT<ELT>::Compare(const CBufferRefT<ELT> &cref) const {
	return m_nSize == cref.m_nSize ? nCompare(cref.GetBuffer()) : 1;
}

template<class ELT>
inline bool CBufferRefT<ELT>::CompareNoCase(const CBufferRefT<ELT> &cref) const {
	return m_nSize == cref.m_nSize ? nCompareNoCase(cref.GetBuffer()) : 1;
}

template<class ELT>
inline ELT CBufferRefT<ELT>::At(int nIndex, ELT def) const {
	return nIndex >= m_nSize ? def : m_pBuffer[nIndex];
}

template<class ELT>
inline ELT CBufferRefT<ELT>::operator[](int nIndex) const {
	return nIndex >= m_nSize ? 0 : m_pBuffer[nIndex];
}

template<class ELT>
const ELT *CBufferRefT<ELT>::GetBuffer() const {
	static const ELT _def[] = {0};
	return m_pBuffer ? m_pBuffer : _def;
}

template<class ELT>
inline int CBufferRefT<ELT>::GetSize() const {
	return m_nSize;
}

template<class ELT>
CBufferRefT<ELT>::~CBufferRefT() {
}
...
//
template<class ELT>
class CBufferT: public CBufferRefT<ELT> {
public:
...
//
template<class ELT>
CBufferT<ELT>::CBufferT(const ELT *pcsz, int length):
	CBufferRefT<ELT>(0, length) {
...
template<class ELT>
CBufferT<ELT>::CBufferT(const ELT *pcsz):
	CBufferRefT<ELT>(pcsz) {
	m_nMaxLength = CBufferRefT<ELT>::m_nSize + 1;
...
template<class ELT>
inline ELT &CBufferT<ELT>::operator[](int nIndex) {
	return CBufferRefT<ELT>::m_pBuffer[nIndex];
}

template<class ELT>
inline const ELT &CBufferT<ELT>::operator[](int nIndex) const {
	return CBufferRefT<ELT>::m_pBuffer[nIndex];
}

template<class ELT>
void CBufferT<ELT>::Append(const ELT *pcsz, int length, int eol) {
...
template<class ELT>
inline void CBufferT<ELT>::Append(ELT el, int eol) {
	Append(&el, 1, eol);
}

template<class ELT>
void CBufferT<ELT>::Push(ELT el) {
	// Realloc
...
template<class ELT>
void CBufferT<ELT>::Push(const CBufferRefT<ELT> &buf) {
...
template<class ELT>
inline bool CBufferT<ELT>::Pop(ELT &el) {
...
template<class ELT>
int CBufferT<ELT>::Pop(CBufferT<ELT> &buf) {
	int size = 0, res = 1;
...
template<class ELT>
inline bool CBufferT<ELT>::Peek(ELT &el) const {
	if(CBufferRefT<ELT>::m_nSize > 0) {
...
template<class ELT>
const ELT *CBufferT<ELT>::GetBuffer() const {
	static const ELT _def[] = {0};
	return CBufferRefT<ELT>::m_pBuffer ? CBufferRefT<ELT>::m_pBuffer : _def;
}

template<class ELT>
ELT *CBufferT<ELT>::GetBuffer() {
	static const ELT _def[] = {0};
	return CBufferRefT<ELT>::m_pBuffer ? CBufferRefT<ELT>::m_pBuffer : (ELT *)_def;
}

template<class ELT>
ELT *CBufferT<ELT>::Detach() {
...
template<class ELT>
void CBufferT<ELT>::Prepare(int index, int fill) {
...
template<class ELT>
inline void CBufferT<ELT>::Restore(int size) {
	SetMaxLength(size);
...
template<class ELT>
CBufferT<ELT>::~CBufferT() {
	if(CBufferRefT<ELT>::m_pBuffer != 0)
		free(CBufferRefT<ELT>::m_pBuffer);
}

template<class T>
class CSortedBufferT: public CBufferT<T> {
public:
...
template<class T>
CSortedBufferT<T>::CSortedBufferT(int reverse) {
	m_fncompare	   = reverse ? compareReverseT : compareT;
	m_bSortFreezed = 0;
}

template<class T>
CSortedBufferT<T>::CSortedBufferT(int (*compare)(const void *, const void *)) {
	m_fncompare	   = compare;
	m_bSortFreezed = 0;
}

template<class T>
void CSortedBufferT<T>::Add(const T &rT) {
	if(m_bSortFreezed != 0) {
...
template<class T>
void CSortedBufferT<T>::Add(const T *pT, int nSize) {
...
template<class T>
int CSortedBufferT<T>::FindAs(const T &rT, int (*compare)(const void *, const void *)) {
...
template<class T>
int CSortedBufferT<T>::Remove(const T &rT) {
	int pos = Find(rT);
	if(pos >= 0)
		CBufferT<T>::Remove(pos);
	return pos;
}

template<class T>
inline void CSortedBufferT<T>::RemoveAll() {
	CBufferT<T>::Restore(0);
}

template<class T>
void CSortedBufferT<T>::SortUnFreeze() {
	if(m_bSortFreezed != 0) {
		m_bSortFreezed = 0;
		qsort(CBufferRefT<T>::m_pBuffer, CBufferRefT<T>::m_nSize, sizeof(T), m_fncompare);
	}
...
template<class T>
int CSortedBufferT<T>::compareT(const void *elem1, const void *elem2) {
...
template<class T>
int CSortedBufferT<T>::compareReverseT(const void *elem1, const void *elem2) {
	if(*(const T *)elem1 == *(const T *)elem2)
		return 0;
	else if(*(const T *)elem1 > *(const T *)elem2)
		return -1;
...
class ElxInterface {
public:
	virtual bool Match(CContext *pContext) const	 = 0;
	virtual bool MatchNext(CContext *pContext) const = 0;
...
//
template<int x>
class CAlternativeElxT: public ElxInterface {
public:
	bool Match(CContext *pContext) const;
	bool MatchNext(CContext *pContext) const;
...
//
template<int x>
class CAssertElxT: public ElxInterface {
public:
	bool Match(CContext *pContext) const;
	bool MatchNext(CContext *pContext) const;
...
class CBackrefElxT: public ElxInterface {
public:
	bool Match(CContext *pContext) const;
	bool MatchNext(CContext *pContext) const;
...
//
template<class CHART>
CBackrefElxT<CHART>::CBackrefElxT(int nnumber, int brightleft, int bignorecase) {
	m_nnumber	  = nnumber;
...
template<class CHART>
bool CBackrefElxT<CHART>::Match(CContext *pContext) const {
...
template<class CHART>
bool CBackrefElxT<CHART>::MatchNext(CContext *pContext) const {
	int npos = 0;
...
//
template<class CHART>
class CBoundaryElxT: public ElxInterface {
public:
	bool Match(CContext *pContext) const;
	bool MatchNext(CContext *pContext) const;
...
protected:
	static bool IsWordChar(CHART ch);
...
//
template<class CHART>
CBoundaryElxT<CHART>::CBoundaryElxT(int ntype, int byes) {
	m_ntype = ntype;
	m_byes	= byes;
}

template<class CHART>
bool CBoundaryElxT<CHART>::Match(CContext *pContext) const {
	const CHART *pcsz = (const CHART *)pContext->m_pMatchString;
	int			 npos = pContext->m_nCurrentPos;
	int			 tlen = pContext->m_pMatchStringLength;
...
template<class CHART>
bool CBoundaryElxT<CHART>::MatchNext(CContext *) const {
	return 0;
}

template<class CHART>
inline bool CBoundaryElxT<CHART>::IsWordChar(CHART ch) {
...
//
template<class CHART>
class CBracketElxT: public ElxInterface {
public:
	bool Match(CContext *pContext) const;
	bool MatchNext(CContext *pContext) const;
...
public:
	...
	static bool CheckCaptureIndex(int &index, CContext *pContext, int number);
...
template<class CHART>
CBracketElxT<CHART>::CBracketElxT(int nnumber, int bright) {
	m_nnumber	= nnumber;
	m_bright	= bright;
	m_balancing = -1;
}
...
template<class CHART>
inline bool CBracketElxT<CHART>::CheckCaptureIndex(int &index, CContext *pContext, int number) {
...
//
template<class CHART>
bool CBracketElxT<CHART>::Match(CContext *pContext) const {
	// check, for named
	if(m_nnumber < 0)
		return 0;
...
template<class CHART>
bool CBracketElxT<CHART>::MatchNext(CContext *pContext) const {
	int index = pContext->m_captureindex[m_nnumber];
	if(!CheckCaptureIndex(index, pContext, m_nnumber)) {
		return 0;
	}
...
//
template<class CHART>
class CDelegateElxT: public ElxInterface {
public:
	bool Match(CContext *pContext) const;
	bool MatchNext(CContext *pContext) const;
...
template<class CHART>
CDelegateElxT<CHART>::CDelegateElxT(int ndata) {
	m_pelx	= 0;
	m_ndata = ndata;
}

template<class CHART>
bool CDelegateElxT<CHART>::Match(CContext *pContext) const {
	if(m_pelx != 0) {
...
template<class CHART>
bool CDelegateElxT<CHART>::MatchNext(CContext *pContext) const {
	if(m_pelx != 0)
...
//
template<int x>
class CEmptyElxT: public ElxInterface {
public:
	bool Match(CContext *pContext) const;
	bool MatchNext(CContext *pContext) const;
...
//
template<int x>
class CGlobalElxT: public ElxInterface {
public:
	bool Match(CContext *pContext) const;
	bool MatchNext(CContext *pContext) const;
...
//
template<int x>
class CRepeatElxT: public ElxInterface {
public:
	bool Match(CContext *pContext) const;
	bool MatchNext(CContext *pContext) const;
...
protected:
	bool MatchFixed(CContext *pContext) const;
	bool MatchNextFixed(CContext *pContext) const;
	bool MatchForward(CContext *pContext) const {
...
//
template<int x>
class CGreedyElxT: public CRepeatElxT<x> {
public:
	bool Match(CContext *pContext) const;
	bool MatchNext(CContext *pContext) const;
...
protected:
	bool MatchVart(CContext *pContext) const;
	bool MatchNextVart(CContext *pContext) const;
...
//
template<int x>
class CIndependentElxT: public ElxInterface {
public:
	bool Match(CContext *pContext) const;
	bool MatchNext(CContext *pContext) const;
...
//
template<int x>
class CListElxT: public ElxInterface {
public:
	bool Match(CContext *pContext) const;
	bool MatchNext(CContext *pContext) const;
...
//
template<class CHART>
class CPosixElxT: public ElxInterface {
public:
	bool Match(CContext *pContext) const;
	bool MatchNext(CContext *pContext) const;
...
//
template<class CHART>
CPosixElxT<CHART>::CPosixElxT(const char *posix, int brightleft) {
	m_brightleft = brightleft;
...
template<class CHART>
inline int _lt256(CHART c) {
	return c >= 0 && c < 256;
}

template<class CHART>
bool CPosixElxT<CHART>::Match(CContext *pContext) const {
	if(m_posixfun == 0)
		return 0;
	...
	bool bsucc = _lt256(ch) && (*m_posixfun)(ch);
...
template<class CHART>
bool CPosixElxT<CHART>::MatchNext(CContext *pContext) const {
	pContext->m_nCurrentPos -= m_brightleft ? -1 : 1;
	return 0;
}
...
//
template<int x>
class CPossessiveElxT: public CGreedyElxT<x> {
public:
	bool Match(CContext *pContext) const;
	bool MatchNext(CContext *pContext) const;
...
//
template<class CHART>
class CRangeElxT: public ElxInterface {
public:
	bool Match(CContext *pContext) const;
	bool MatchNext(CContext *pContext) const;
...
public:
	bool IsContainChar(CHART ch) const;
...
//
template<class CHART>
CRangeElxT<CHART>::CRangeElxT(int brightleft, int byes) {
	m_brightleft = brightleft;
	m_byes		 = byes;
}

template<class CHART>
bool CRangeElxT<CHART>::Match(CContext *pContext) const {
	int tlen = pContext->m_pMatchStringLength;
	int npos = pContext->m_nCurrentPos;
...
template<class CHART>
bool CRangeElxT<CHART>::IsContainChar(CHART ch) const {
	int bsucc = 0, i;
...
template<class CHART>
bool CRangeElxT<CHART>::MatchNext(CContext *pContext) const {
	pContext->m_nCurrentPos -= m_brightleft ? -1 : 1;
	return 0;
}
...
//
template<int x>
class CReluctantElxT: public CRepeatElxT<x> {
public:
	bool Match(CContext *pContext) const;
	bool MatchNext(CContext *pContext) const;
...
//
template<class CHART>
class CStringElxT: public ElxInterface {
public:
	bool Match(CContext *pContext) const;
	bool MatchNext(CContext *pContext) const;
...
template<class CHART>
bool CStringElxT<CHART>::Match(CContext *pContext) const {
	const CHART *pcsz = (const CHART *)pContext->m_pMatchString;
	int			 npos = pContext->m_nCurrentPos;
	int			 tlen = pContext->m_pMatchStringLength;
...
template<class CHART>
bool CStringElxT<CHART>::MatchNext(CContext *pContext) const {
	int slen = m_szPattern.GetSize();
...
//
template<class CHART>
class CConditionElxT: public ElxInterface {
public:
	bool Match(CContext *pContext) const;
	bool MatchNext(CContext *pContext) const;
...
template<class CHART>
CConditionElxT<CHART>::CConditionElxT() {
	m_nnumber = -1;
}

template<class CHART>
bool CConditionElxT<CHART>::Match(CContext *pContext) const {
	// status
...
template<class CHART>
bool CConditionElxT<CHART>::MatchNext(CContext *pContext) const {
	// pop
...
protected:
	...
	public:
		...
		inline bool operator==(const CHART_INFO &ci) { return ch == ci.ch && type == ci.type; }
		inline bool operator!=(const CHART_INFO &ci) { return !operator==(ci); }
	};
...
protected:
	...
	struct Snapshot {
		...
		POSIX_FUNC m_quote_fun;
		Snapshot():
			prev(0, 0), curr(0, 0), next(0, 0), nex2(0, 0) {}
	};
...
//
template<class CHART>
CBuilderT<CHART>::CBuilderT():
	m_pattern(0, 0), prev(0, 0), curr(0, 0), next(0, 0), nex2(0, 0) {
	Clear();
}
...
template<class CHART>
int CBuilderT<CHART>::GetNamedNumber(const CBufferRefT<CHART> &named) const {
...
// added (am) 2015-10-29
template<class CHART>
const CBufferRefT<CHART> CBuilderT<CHART>::GetNamedName(int nnumber) const {
...
template<class CHART>
ElxInterface *CBuilderT<CHART>::Build(const CBufferRefT<CHART> &pattern, int flags) {
	// init
	m_pattern		 = pattern;
	m_nNextPos		 = 0;
	m_nCharsetDepth	 = 0;
...
//
template<class CHART>
unsigned int CBuilderT<CHART>::Hex2Int(const CHART *pcsz, int length, int &used) {
...
template<class CHART>
inline ElxInterface *CBuilderT<CHART>::Keep(ElxInterface *pelx) {
	m_objlist.Push(pelx);
	return pelx;
}

template<class CHART>
void CBuilderT<CHART>::MoveNext() {
	// forwards
	prev = curr;
	curr = next;
	next = nex2;
...
int CBuilderT<CHART>::GetNext2() {
	...
	case RCHART('\\'): {
		...
		case RCHART('a'):
			nex2 = CHART_INFO(RCHART('\a'), 0, m_nNextPos, delta);
			break;
		case RCHART('f'):
			nex2 = CHART_INFO(RCHART('\f'), 0, m_nNextPos, delta);
			break;
		case RCHART('n'):
			nex2 = CHART_INFO(RCHART('\n'), 0, m_nNextPos, delta);
			break;
		case RCHART('r'):
			nex2 = CHART_INFO(RCHART('\r'), 0, m_nNextPos, delta);
			break;
		case RCHART('t'):
			nex2 = CHART_INFO(RCHART('\t'), 0, m_nNextPos, delta);
			break;
		case RCHART('v'):
			nex2 = CHART_INFO(RCHART('\v'), 0, m_nNextPos, delta);
			break;
		case RCHART('e'):
			nex2 = CHART_INFO(RCHART(27), 0, m_nNextPos, delta);
			break;
...
template<class CHART>
ElxInterface *CBuilderT<CHART>::GetStockElx(int nStockId) {
	...
	if(pStockElxs[nStockId] == 0) {
		...
		case STOCKELX_WORD: {
			...
			pRange->m_ranges.Push(RCHART('A'));
			pRange->m_ranges.Push(RCHART('Z'));
			pRange->m_ranges.Push(RCHART('a'));
			pRange->m_ranges.Push(RCHART('z'));
			pRange->m_ranges.Push(RCHART('0'));
			pRange->m_ranges.Push(RCHART('9'));
...
template<class CHART>
ElxInterface *CBuilderT<CHART>::BuildAlternative(int vaflags) {
...
template<class CHART>
ElxInterface *CBuilderT<CHART>::BuildList(int &flags) {
...
template<class CHART>
ElxInterface *CBuilderT<CHART>::BuildRepeat(int &flags) {
	// simple
...
template<class CHART>
ElxInterface *CBuilderT<CHART>::BuildSimple(int &flags) {
...
template<class CHART>
ElxInterface *CBuilderT<CHART>::BuildCharset(int &flags) {
	// char
	CHART ch = curr.ch;
	...
	case RCHART('.'):
		return GetStockElx(
			flags & RIGHTTOLEFT ? ((flags & SINGLELINE) ? STOCKELX_DOT_ALL_RIGHTLEFT : STOCKELX_DOT_NOT_ALL_RIGHTLEFT) : ((flags & SINGLELINE) ? STOCKELX_DOT_ALL : STOCKELX_DOT_NOT_ALL));
	...
	case RCHART('['): {
		...
		while(curr != CHART_INFO(0, 1) && curr != CHART_INFO(RCHART(']'), 1)) {
			...
			if(curr.type == 1 && (ch == RCHART('.') || ch == RCHART('w') || ch == RCHART('W') || ch == RCHART('s') || ch == RCHART('S') || ch == RCHART('d') || ch == RCHART('D') ||
...
template<class CHART>
ElxInterface *CBuilderT<CHART>::BuildRecursive(int &flags) {
	// skip '('
	MoveNext();
...
template<class CHART>
ElxInterface *CBuilderT<CHART>::BuildBoundary(int &flags) {
	// char
	CHART ch = curr.ch;
...
template<class CHART>
ElxInterface *CBuilderT<CHART>::BuildBackref(int &flags) {
	// skip '\\' or '\k' or '\g'
	MoveNext();
...
template<class CHART>
int CBuilderT<CHART>::ReadDec(char *&str, unsigned int &dec) {
	int s = 0;
...
//
template<class CHART>
CRegexpT<CHART>::CRegexpT(const CHART *pattern, int flags) {
	Compile(pattern, CBufferRefT<CHART>(pattern).GetSize(), flags);
}

template<class CHART>
CRegexpT<CHART>::CRegexpT(const CHART *pattern, int length, int flags) {
	Compile(pattern, length, flags);
}

template<class CHART>
inline void CRegexpT<CHART>::Compile(const CHART *pattern, int flags) {
	Compile(pattern, CBufferRefT<CHART>(pattern).GetSize(), flags);
}

template<class CHART>
void CRegexpT<CHART>::Compile(const CHART *pattern, int length, int flags) {
	m_builder.Clear();
	if(pattern != 0)
...
template<class CHART>
inline MatchResult CRegexpT<CHART>::MatchExact(const CHART *tstring, CContext *pContext) const {
	return MatchExact(tstring, CBufferRefT<CHART>(tstring).GetSize(), pContext);
}

template<class CHART>
MatchResult CRegexpT<CHART>::MatchExact(const CHART *tstring, int length, CContext *pContext) const {
...
template<class CHART>
MatchResult CRegexpT<CHART>::Match(const CHART *tstring, int start, CContext *pContext) const {
	return Match(tstring, CBufferRefT<CHART>(tstring).GetSize(), start, pContext);
}

template<class CHART>
MatchResult CRegexpT<CHART>::Match(const CHART *tstring, int length, int start, CContext *pContext) const {
	if(m_builder.m_pTopElx == 0)
		return 0;
...
template<class CHART>
MatchResult CRegexpT<CHART>::Match(CContext *pContext) const {
	if(m_builder.m_pTopElx == 0)
		return 0;
...
template<class CHART>
inline CContext *CRegexpT<CHART>::PrepareMatch(const CHART *tstring, int start, CContext *pContext) const {
	return PrepareMatch(tstring, CBufferRefT<CHART>(tstring).GetSize(), start, pContext);
}

template<class CHART>
CContext *CRegexpT<CHART>::PrepareMatch(const CHART *tstring, int length, int start, CContext *pContext) const {
	if(m_builder.m_pTopElx == 0)
		return 0;
...
template<class CHART>
inline int CRegexpT<CHART>::GetNamedGroupNumber(const CHART *group_name) const {
	return m_builder.GetNamedNumber(group_name);
}

// added (am) 2015-10-29
template<class CHART>
inline const CHART *CRegexpT<CHART>::GetNamedGroupName(int group_number) const {
	return m_builder.GetNamedName(group_number).GetBuffer();
}

template<class CHART>
CHART *CRegexpT<CHART>::Replace(const CHART *tstring, const CHART *replaceto, int start, int ntimes, MatchResult *result, CContext *pContext) const {
	int result_length = 0;
...
template<class CHART>
CHART *CRegexpT<CHART>::Replace(const CHART *tstring, int string_length, const CHART *replaceto, int to_length, int &result_length, int start, int ntimes, MatchResult *remote_result, CContext *oContext) const {
	if(m_builder.m_pTopElx == 0)
		return 0;
...
template<class CHART>
inline void CRegexpT<CHART>::ReleaseString(CHART *tstring) {
	if(tstring != 0)
		free(tstring);
}

template<class CHART>
inline void CRegexpT<CHART>::ReleaseContext(CContext *pContext) {
	if(pContext != 0)
		delete pContext;
}
...
//
template<int x>
CAlternativeElxT<x>::CAlternativeElxT() {
}

template<int x>
bool CAlternativeElxT<x>::Match(CContext *pContext) const {
...
template<int x>
bool CAlternativeElxT<x>::MatchNext(CContext *pContext) const {
	if(m_elxlist.GetSize() == 0)
		return 0;
...
//
template<int x>
CAssertElxT<x>::CAssertElxT(ElxInterface *pelx, int byes) {
	m_pelx = pelx;
	m_byes = byes;
}

template<int x>
bool CAssertElxT<x>::Match(CContext *pContext) const {
	int	 nbegin = pContext->m_nCurrentPos;
	int	 nsize	= pContext->m_stack.GetSize();
	int	 ncsize = pContext->m_capturestack.GetSize();
	bool bsucc;
...
template<int x>
bool CAssertElxT<x>::MatchNext(CContext *pContext) const {
	int ncsize = 0;
...
//
template<int x>
CEmptyElxT<x>::CEmptyElxT() {
}

template<int x>
bool CEmptyElxT<x>::Match(CContext *) const {
	return 1;
}

template<int x>
bool CEmptyElxT<x>::MatchNext(CContext *) const {
	return 0;
}
...
//
template<int x>
CGlobalElxT<x>::CGlobalElxT() {
}

template<int x>
bool CGlobalElxT<x>::Match(CContext *pContext) const {
	return pContext->m_nCurrentPos == pContext->m_nBeginPos;
}

template<int x>
bool CGlobalElxT<x>::MatchNext(CContext *) const {
	return 0;
}
...
//
template<int x>
CGreedyElxT<x>::CGreedyElxT(ElxInterface *pelx, int nmin, int nmax):
	CRepeatElxT<x>(pelx, nmin) {
	m_nvart = nmax - nmin;
}

template<int x>
bool CGreedyElxT<x>::Match(CContext *pContext) const {
...
template<int x>
bool CGreedyElxT<x>::MatchNext(CContext *pContext) const {
...
template<int x>
bool CGreedyElxT<x>::MatchVart(CContext *pContext) const {
	int n		 = 0;
...
template<int x>
bool CGreedyElxT<x>::MatchNextVart(CContext *pContext) const {
...
//
template<int x>
CIndependentElxT<x>::CIndependentElxT(ElxInterface *pelx) {
	m_pelx = pelx;
}

template<int x>
bool CIndependentElxT<x>::Match(CContext *pContext) const {
	int nbegin = pContext->m_nCurrentPos;
	int nsize  = pContext->m_stack.GetSize();
	int ncsize = pContext->m_capturestack.GetSize();
...
template<int x>
bool CIndependentElxT<x>::MatchNext(CContext *pContext) const {
...
//
template<int x>
CListElxT<x>::CListElxT(int brightleft) {
	m_brightleft = brightleft;
}

template<int x>
bool CListElxT<x>::Match(CContext *pContext) const {
	if(m_elxlist.GetSize() == 0)
		return 1;
...
template<int x>
bool CListElxT<x>::MatchNext(CContext *pContext) const {
	if(m_elxlist.GetSize() == 0)
		return 0;
...
//
template<int x>
MatchResultT<x>::MatchResultT(CContext *pContext, int nMaxNumber) {
	if(pContext != 0) {
		...
		for(int n = 0; n <= nMaxNumber; n++) {
			...
			//if( index < 0 ) continue;
			if(!CBracketElxT<char>::CheckCaptureIndex(index, pContext, n))
				continue;
...
template<int x>
inline int MatchResultT<x>::IsMatched() const {
	return m_result.At(0, 0);
}

template<int x>
inline int MatchResultT<x>::MaxGroupNumber() const {
	return m_result.At(1, 0);
}

template<int x>
inline int MatchResultT<x>::GetStart() const {
	return m_result.At(2, -1);
}

template<int x>
inline int MatchResultT<x>::GetEnd() const {
	return m_result.At(3, -1);
}

template<int x>
inline int MatchResultT<x>::GetGroupStart(int nGroupNumber) const {
	return m_result.At(2 + nGroupNumber * 2, -1);
}

template<int x>
inline int MatchResultT<x>::GetGroupEnd(int nGroupNumber) const {
	return m_result.At(2 + nGroupNumber * 2 + 1, -1);
}

template<int x>
MatchResultT<x> &MatchResultT<x>::operator=(const MatchResultT<x> &result) {
	m_result.Restore(0);
...
//
template<int x>
CPossessiveElxT<x>::CPossessiveElxT(ElxInterface *pelx, int nmin, int nmax):
	CGreedyElxT<x>(pelx, nmin, nmax) {
}

template<int x>
bool CPossessiveElxT<x>::Match(CContext *pContext) const {
	int nbegin = pContext->m_nCurrentPos;
	int nsize  = pContext->m_stack.GetSize();
	int ncsize = pContext->m_capturestack.GetSize();
	int bsucc  = 1;
...
template<int x>
bool CPossessiveElxT<x>::MatchNext(CContext *pContext) const {
	int nbegin = 0, ncsize = 0;
...
//
template<int x>
CReluctantElxT<x>::CReluctantElxT(ElxInterface *pelx, int nmin, int nmax):
	CRepeatElxT<x>(pelx, nmin) {
	m_nvart = nmax - nmin;
}

template<int x>
bool CReluctantElxT<x>::Match(CContext *pContext) const {
	if(!CRepeatElxT<x>::MatchFixed(pContext))
		return 0;
...
template<int x>
bool CReluctantElxT<x>::MatchNext(CContext *pContext) const {
	if(MatchNextVart(pContext))
		return 1;
...
template<int x>
bool CReluctantElxT<x>::MatchVart(CContext *pContext) const {
...
template<int x>
bool CReluctantElxT<x>::MatchNextVart(CContext *pContext) const {
...
//
template<int x>
CRepeatElxT<x>::CRepeatElxT(ElxInterface *pelx, int ntimes) {
	m_pelx	 = pelx;
	m_nfixed = ntimes;
}

template<int x>
bool CRepeatElxT<x>::Match(CContext *pContext) const {
	return MatchFixed(pContext);
}

template<int x>
bool CRepeatElxT<x>::MatchNext(CContext *pContext) const {
	return MatchNextFixed(pContext);
}

template<int x>
bool CRepeatElxT<x>::MatchFixed(CContext *pContext) const {
	if(m_nfixed == 0)
		return 1;
...
template<int x>
bool CRepeatElxT<x>::MatchNextFixed(CContext *pContext) const {
	if(m_nfixed == 0)
		return 0;
