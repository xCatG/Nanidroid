...
open class ShellSurface {
    ...
    open inner class Animation { @JvmField var interval = 0; @JvmField var id: String = ""; @JvmField var exclusive = false; @JvmField var hasError = false; @JvmField var frames: MutableList<AnimationFrame>? = null; @JvmField var animation: AnimationDrawable? = null
        ...
        fun addFrame(index: Int, frame: AnimationFrame) { val list=frames ?: ArrayList<AnimationFrame>().also { frames=it }; while(list.size < index) list.add(frame); list.add(index, frame) }
        open fun getAnimation(res:Resources, mgr:SurfaceManager?=null):AnimationDrawable? { animation?.let{return it};if(hasError)return null;val result=AnimationDrawable();for(frame in frames.orEmpty()){val drawable=frame.getDrawable(res,mgr) ?: run {hasError=true;return null};result.addFrame(drawable,frame.time)};return result.also{animation=it} }
    }
    ...
    private fun handlePattern(id:String, sequence:String, filename:String, wait:String, type:String, x:String?, y:String?) { try { val frame=AnimationFrame(); frame.time=wait.toInt(); frame.startX=x?.toInt() ?: 0; frame.startY=y?.toInt() ?: 0; if(filename.equals("-1",true)) frame.frameType=TYPE_RESET else { frame.sid=filename.toInt().toString(); frame.frameType=lookupPatternType(type) }; addFrameToAnimation(id,sequence.toInt(),frame) } catch (_: Exception) {} }
    ...
    fun getAnimationFrameCount(patternId:Int): Int = animationTable?.get(patternId.toString())?.frames?.size ?: 0
    ...
    private fun handleOptions(value:String) { try { AnalyticsUtils.getInstance(null).trackEvent(Setup.ANA_ERR,"surface parse not support",value,-1) } catch (_:Exception) {} }
    private fun addAltAnimation(id:String, ids:String, splitter:String) = addAltAnimation(id,ids.split(splitter).toTypedArray())
