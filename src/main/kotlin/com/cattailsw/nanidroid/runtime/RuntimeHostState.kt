...
internal object RuntimeHostReducer {
    ...
    private fun setResumed(
        ...
        if (!state.isCurrent(lease)) return unchanged(state)
        val nextResumed = if (resumed) state.resumed + lease else state.resumed - lease
        var next = state.copy(resumed = nextResumed)
        val effects = mutableListOf<RuntimeHostEffect>()
        if (!resumed && state.topResumed == lease) {
            next = loseTopHost(next)
            appendBackpressureRelease(state, effects)
        }
        return RuntimeHostTransition(next, effects)
    ...
    private fun setTopResumed(
        ...
        if (!topResumed) {
            return revokeHost(state, lease, unregister = false)
        }
        if (!state.isCurrent(lease)) return unchanged(state)
        if (lease !in state.resumed || state.topResumed == lease) return unchanged(state)

        val effects = mutableListOf<RuntimeHostEffect>()
        var next = if (state.topResumed != null) {
            appendBackpressureRelease(state, effects)
            loseTopHost(state)
        } else {
            state
        }
        next = next.copy(topResumed = lease)
    ...
    private fun unregister(state: RuntimeHostState, lease: RuntimeHostLease): RuntimeHostTransition {
        return revokeHost(state, lease, unregister = true)
    }
    ...
    private fun revokeHost(
        ...
        if (lease.hostEpoch == currentEpoch && !unregister && state.topResumed != lease) {
            return unchanged(state)
        }
        var next = state.copy(
            registeredEpochs = if (lease.hostEpoch > currentEpoch) {
                state.registeredEpochs + (lease.hostId to lease.hostEpoch)
            } else {
                state.registeredEpochs
            },
            resumed = if (lease.hostEpoch > currentEpoch) {
                state.resumed.filterNot { it.hostId == lease.hostId }.toSet()
            } else {
                state.resumed - lease
            },
        )
        val effects = mutableListOf<RuntimeHostEffect>()
        if (state.topResumed?.hostId == lease.hostId &&
            (state.topResumed == lease || lease.hostEpoch > currentEpoch)
        ) {
            next = loseTopHost(next)
            appendBackpressureRelease(state, effects)
        }
    ...
    private fun RuntimeHostState.isCurrent(lease: RuntimeHostLease): Boolean =
        registeredEpochs[lease.hostId] == lease.hostEpoch
