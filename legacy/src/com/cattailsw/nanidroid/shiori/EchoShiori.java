...
public class EchoShiori implements Shiori {
    ...
    public EchoShiori() { for (String s : ignoreId) igTable.add(s); }
    public String getModuleName() { return "EchoShiori"; }
    private static String[] ignoreId = {"OnSecondChange"};

    public String request(String req) { parseRequest(req); return genResponse(); }
    protected String genResponse() {
        if (reqTable != null) {
            if (!matchIgnoreId(reqTable.get("id"))) {
    ...
    private boolean matchIgnoreId(String in) { return igTable.contains(in); }
    ...
    private void parseRequest(String req) {
        ...
        try { header = br.readLine(); } catch (Exception e) { return; }
        String line = null;
        reqTable = new Hashtable<String, String>();
        while (true) {
            try { line = br.readLine(); } catch (Exception e) { line = null; }
            if (line == null) break;
            if (line.indexOf(": ") == -1) {
                if (line.length() > 0) Log.d(TAG, "got a non recognized line\n" + line);
                continue;
            }
