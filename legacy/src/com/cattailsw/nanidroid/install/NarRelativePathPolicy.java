...
final class NarRelativePathPolicy {
    ...
    static Result normalize(String raw) {
        if (raw == null || raw.length() == 0 || raw.startsWith("/")
                || raw.indexOf('\\') >= 0 || !validUnicode(raw)) {
            return Result.failure(Error.INVALID_PATH);
        }
        String[] components = raw.split("/", -1);
        if (components.length > MAX_DEPTH) return Result.failure(Error.PATH_DEPTH_LIMIT);
        StringBuilder normalized = new StringBuilder();
        try {
            for (String component : components) {
                if (component.length() == 0 || ".".equals(component)
                        || "..".equals(component) || component.indexOf(':') >= 0
                        || containsControl(component)) return Result.failure(Error.INVALID_PATH);
            ...
            return Result.success(path, collisionKey(path));
    ...
    static String collisionKey(String value) {
        ...
        return Normalizer.normalize(nfc.toLowerCase(Locale.US), Normalizer.Form.NFC);
    }
    ...
    enum Error { INVALID_PATH, PATH_DEPTH_LIMIT, PATH_LENGTH_LIMIT, COMPONENT_LENGTH_LIMIT }

    static final class Result {
        private final String normalized;
        private final String key;
        private final Error error;
        private Result(String normalized, String key, Error error) {
            this.normalized = normalized; this.key = key; this.error = error;
        }
        private static Result success(String normalized, String key) { return new Result(normalized, key, null); }
        private static Result failure(Error error) { return new Result(null, null, error); }
