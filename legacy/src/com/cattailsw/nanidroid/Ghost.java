...
/** Frozen Java domain owner restored only inside the disposable Ant build. */
public class Ghost {
	...
	mgr = new SurfaceManager(ghostDirName);
	mCtx = ctx;
	loadGhostInfo();
	incrementCreateCount();
    }

    public Ghost(String ghostPath) { this(ghostPath, null); }
    public boolean ghostError(){ return error; }
	...
	DescReader ghost_dr = new DescReader(master_ghost + "descript.txt");
	String master_shell = rootPath + "/shell/master/";
	DescReader shell_dr = new DescReader(master_shell + "descript.txt");
	...
	if ( error == false ) error = sr.error;
	shiori = ShioriFactory.getInstance().getShiori( master_ghost, ghostDesc, mCtx );
    }

    public void unload() { shiori.unloadShiori(); }
    public String getGhostId() { return ghostDirName; }
    public String getGhostDirName() { return ghostDirName; }
    public String getGhostPath(){ return rootPath; }
    public String getGhostName() { return ghostDesc.get("name"); }
    public String getShellName() { return shellDesc == null ? "master" : shellDesc.get("name"); }
    public String getCrafterName() {
	if ( ghostDesc.get("craftmanw") != null ) return ghostDesc.get("craftmanw");
	return ghostDesc.get("craftman");
    }
    public String getSakuraName() { return ghostDesc.get("sakura.name"); }
    public String getKeroName(){ return ghostDesc.get("kero.name"); }
	...
	if ( shiori == null ) return new ShioriResponse("SHIORI/2.0 500 Internal Server Error");
	StringBuffer sb = new StringBuffer();
	sb.append("GET SHIORI/3.0\r\nSender: "+ Setup.NANIDROID + "\r\n");
	sb.append("ID: " + event + "\r\n");
	sb.append("SecurityLevel: local\r\n");
	if ( ref != null ) {
	    for ( int i = 0; i < ref.length; i++ ) sb.append("Reference"+i+": " + ref[i] + "\r\n");
	}
	sb.append("\r\n");
