...
string	ShioriPlugins::request(const string& iCallName, const strvec& iArguments, strvec& oResults, const string& iSecurityLevel) {
	...
	else 
	{
		...
		case 204:
			break;
		case 400:
			PluginError(theCallData.mDllPath + ": 400 Bad Request / 呼び出しの不備");
			break;
		case 500:
			PluginError(theCallData.mDllPath + ": 500 Internal Server Error / saori内でのエラー");
			break;
		default:
			PluginError(theCallData.mDllPath + ": " + itos(return_code) + "? / 定義されていないステータスを返しました。");
			break;
		}
