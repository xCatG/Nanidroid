...
bool	Satori::Call(const string& iName, string& oResult) {
	...
	// SAORI対応, 内蔵関数呼び出しもここで
	{
		...
		else
		{
			...
			for ( strvec::iterator i=theResults.begin() ; i!=theResults.end() ; ++i ) {
				if ( i->size()==0 )
					variables.erase( string("Ｓ")+int2zen(id++));
				else
					variables[string("Ｓ")+int2zen(id++)] = *i;
			}
		}
	}
	...
	else if ( compare_head(iName, "前回終了時サーフェス") && aredigits(iName.c_str()+20) ) {
	...
	else if ( compare_head(iName, "文「") && compare_tail(iName, "」の存在") ) {
	...
	else if ( compare_head(iName, "単語群「") && compare_tail(iName, "」の存在") ) {
